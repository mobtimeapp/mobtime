...
export const summary = (props) => {
  ...
  if (index >= 0) {
    ...
    if (!isSubGoal(incomplete.goal)) {
      for (
        let i = index + 1;
        i < props.goals.length && isSubGoal(props.goals[i]) &&
        !incomplete.subGoal;
        i++
      ) {
  ...
  return h('div', {
    ...
    h('ol', {}, [
      ...props.settings.mobOrder.split(',').map((position, index) => {
        return h('li', { class: 'mb-2', 'data-mobposition': position }, [
          h(
            'div',
            { class: 'text-xs leading-none uppercase text-slate-600' },
            text(position),
          ),
    ...
    h('div', {}, [
      ...
      h(
        'div',
        { class: 'text-xs uppercase text-slate-600' },
        text('Turn Controls'),
      ),
      h('div', { class: 'grid grid-cols-3' }, [
        h('button', {
          'data-timercontrol': 'start',
          type: 'button',
          class: [
